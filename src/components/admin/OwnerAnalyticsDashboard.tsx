...
import {
  ...
  Loader2, TrendingUp, Activity, Send, Bell, BarChart3, Percent, PieChart as PieIcon, FileSpreadsheet, Receipt
} from 'lucide-react';
import {
...
export default function OwnerAnalyticsDashboard({ organizationId }: { organizationId?: string }) {
  ...
  const handleSendAlert = async (memberId: string, memberName: string) => {
    ...
    try {
      ...
      setSendingAlerts(prev => { const next = new Set(prev); next.delete(memberId); return next; });
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Tabs value={subTab} onValueChange={setSubTab}>
        <TabsList className="grid w-full grid-cols-4 h-auto">
          <TabsTrigger value="overview" className="gap-1 text-[10px] sm:text-xs py-2">
            <BarChart3 className="w-3.5 h-3.5 shrink-0" />
            <span className="hidden sm:inline">Overview</span>
          </TabsTrigger>
          <TabsTrigger value="demographics" className="gap-1 text-[10px] sm:text-xs py-2">
            <PieIcon className="w-3.5 h-3.5 shrink-0" />
            <span className="hidden sm:inline">Members</span>
          </TabsTrigger>
          <TabsTrigger value="tax" className="gap-1 text-[10px] sm:text-xs py-2">
            <Receipt className="w-3.5 h-3.5 shrink-0" />
            <span className="hidden sm:inline">Tax</span>
          </TabsTrigger>
          <TabsTrigger value="reports" className="gap-1 text-[10px] sm:text-xs py-2">
          ...
          <Card className="glass-card">
            <CardHeader className="pb-2">
              ...
              <div className="flex gap-3 mt-1">
                <Badge variant="outline" className="text-[10px]">Active: {owner.revenue.totalActiveMembers}</Badge>
                <Badge variant="outline" className="text-[10px]">Total: {owner.revenue.totalMembers}</Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="h-40">
            ...
            <Card className="glass-card border-orange-500/30">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {sendingAlerts.size > 0 ? <Loader2 className="w-3 h-3 animate-spin" /> : <Bell className="w-3 h-3" />}
                    Alert All
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-2">
                {analytics.inactiveMembers.slice(0, 15).map((member) => (
                  <div key={member.user_id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30">
                    ...
                    >
                      {sendingAlerts.has(member.user_id) ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                    </Button>
                ...
                {analytics.inactiveMembers.length > 15 && (
                  <p className="text-xs text-muted-foreground text-center">+{analytics.inactiveMembers.length - 15} more</p>
                )}
              </CardContent>
            </Card>
          )}
